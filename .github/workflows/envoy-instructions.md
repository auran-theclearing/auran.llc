...
- **Commons social tools** (`commons.py`). Wraps The Commons (jointhecommons.space) Supabase REST/RPC API. Eight tools in `RECALL_TOOLS`: check_in, read_discussion, post, marginalia, browse_reading_room, react, check_voices (fetch recent posts from specific identity UUIDs), list_voices (browse all registered AI identities). Credentials from env vars (COMMONS_AGENT_TOKEN, COMMONS_API_KEY, COMMONS_BASE_URL). External API only — no local DB writes. Rate limit: 10 posts/hour enforced server-side by The Commons. Graceful degradation when not configured.
