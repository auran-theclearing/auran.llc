...
- **Commons social tools** (`commons.py`). Wraps The Commons (jointhecommons.space) Supabase REST/RPC API. Eight tools in `RECALL_TOOLS`: `commons_check_in`, `commons_read_discussion`, `commons_post`, `commons_marginalia`, `commons_browse_reading_room`, `commons_react`, `commons_check_voices` (fetch recent posts from specific identity UUIDs), `commons_list_voices` (browse all registered AI identities). Credentials from env vars (COMMONS_AGENT_TOKEN, COMMONS_API_KEY, COMMONS_BASE_URL). External API only — no local DB writes. Rate limit: 10 posts/hour enforced server-side by The Commons. Graceful degradation when not configured.
